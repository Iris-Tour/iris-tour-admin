        ...
        (file: File | FileMetadata): string | null => {
            if (file instanceof File) {
                if (file.size > maxSize) {
                    return `File "${
                        file.name
                    }" dépasse la taille maximale de ${formatBytes(maxSize)}.`;
                }
            } else {
                if (file.size > maxSize) {
                    return `File "${
    ...
    const addFiles = useCallback(
        (newFiles: FileList | File[]) => {
            ...
            if (
                ...
                errors.push(
                    `Vous pouvez téléverser au maximum ${maxFiles} fichiers.`
                );
                setState((prev) => ({ ...prev, errors }));
                return;
            }
            ...
            newFilesArray.forEach((file) => {
                ...
                if (file.size > maxSize) {
                    errors.push(
                        multiple
                            ? `Certains fichiers dépassent la taille maximale ${formatBytes(
                                  maxSize
                              )}.`
                            : `Ce fichier dépasse la taille maximale ${formatBytes(
                                  maxSize
                              )}.`
                    );
                    return;
